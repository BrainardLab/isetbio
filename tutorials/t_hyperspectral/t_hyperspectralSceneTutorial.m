%% Illustrate ISETBio processing of hyperspectral scenes
%
% Description:
%    Illustrates how to:
%       - load a fetch a hyperspectral image
%       - generate a corresponding ISETbio scene
%       - generate a corresponding retinal optical image using
%         wavefront-based optics from a real subject.
%       - generate a realistic cone mosaic
%       - compute cone excitations for the given mosaic and scene to either
%         a statically viewed scene, or dynamically viewed scene (i.e. in
...
%    08/12/19  jnm  Formatting pass

%% Close all open figures
% close all

%% Add subfolders to the path
% This allows us to have access to the supporting functions
% addpath(genpath(pwd));
...
if (loadSceneFromDisk)
    theScene = fetchScene(sceneNo, 'origin', './resources', ...
        'database', sceneDataBase);
else
    theScene = fetchScene(sceneNo, 'origin', 'remote', ...
        'database', sceneDataBase, 'destination', '');
end

    
%% Parameters for the human optics
oiParams = struct('opticsModel', 'ThibosDefaultSubject3MMPupil', ...
    'wavefrontSpatialSamples', 261 * 2 + 1, ...
    'pupilDiamMm', 3.0, 'umPerDegree', 300);
...
visualizePSFfromOI(theOI, oiParams.umPerDegree, ...
    ...
    'visualizedWavelengths', visualizedWaveBands, 'rows', 3, 'cols', 3, ...
    'labelLastPSF', false, 'displayWavelengthInTitle', ~false);
...
%% Visualize the scene
% VisualiW the spatial spectral radiance of the scene at the selected
% wavelength bands
figNo = 3;
...
%% Visualize the image
% Visualize the spatial spectral radiance of the retinal image at the
% selected wavelength bands
figNo = 4;
visualizeSpatialSpectralRadianceOfRetinalImage(...
    theOI, visualizedWaveBands, visualizedXRangeDegs, ...
    visualizedYRangeDegs, figNo);
...
%% Specify how many eye movements to generate.
% Specify 1 for no eye movements.
eyeMovementsNum = 50;
...
eyeMovementOffset = reshape(round(theConeMosaic.micronsPerDegree * ...
    eyeMovementOffsetDegs / coneApertureMicrons), [1 1 2]);
